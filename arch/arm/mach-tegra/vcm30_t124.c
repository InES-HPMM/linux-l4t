...
/*
 * Select the debug uart port based on
 ...
 */
void __init tegra_vcm30_t124_debug_uart_init(enum debug_uart_port dfl_dbg_port)
{
	struct platform_device *hs_debug_device;
	int debug_port_id;

	switch (dfl_dbg_port) {
	case UARTA_DEBUG:
		tegra_uarta_device.dev.platform_data = &vcm30_t124_uarta_pdata;
		hs_debug_device = &tegra_uarta_device;
		break;

	case UARTB_DEBUG:
		tegra_uartb_device.dev.platform_data = &vcm30_t124_uartb_pdata;
		hs_debug_device = &tegra_uartb_device;
		break;

	case UARTC_DEBUG:
		tegra_uartc_device.dev.platform_data = &vcm30_t124_uartc_pdata;
		hs_debug_device = &tegra_uartc_device;
		break;

	case UARTD_DEBUG:
		tegra_uartd_device.dev.platform_data = &vcm30_t124_uartd_pdata;
		hs_debug_device = &tegra_uartd_device;
		break;

	default:
		pr_warn("Uart port %d is not supported\n", dfl_dbg_port);
		return;
	}

	if (!is_tegra_debug_uartport_hs()) {
		debug_port_id = uart_console_debug_init(dfl_dbg_port);
		if (debug_port_id < 0)
			return;
		...
		if (!tegra_is_port_available_from_dt(dfl_dbg_port))
			platform_device_register(hs_debug_device);
	}
