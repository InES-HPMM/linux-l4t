...
int __init loki_regulator_init(void)
{
	...
	if (!(bi.board_id == BOARD_P2530 && bi.sku == BOARD_SKU_FOSTER)) {
		for (i = 0; i < ARRAY_SIZE(palmas_ldo5_supply); i++) {
			struct regulator_consumer_supply *reg =
						 &palmas_ldo5_supply[i];
			if ((strncmp(reg->supply, "avdd_hdmi_pll", 13) == 0)
			   && (strncmp(reg->dev_name, "tegradc.1", 9) == 0)) {
				reg->dev_name = "tegradc.0";
				break;
			}
		}
		for (i = 0; i < ARRAY_SIZE(palmas_ldousb_supply); i++) {
			struct regulator_consumer_supply *reg =
						&palmas_ldousb_supply[i];
			if ((strncmp(reg->supply, "avdd_hdmi", 9) == 0)
			   && (strncmp(reg->dev_name, "tegradc.1", 9) == 0)) {
				reg->dev_name = "tegradc.0";
				break;
			}
		...
		for (i = 0; i < ARRAY_SIZE(palmas_ldousb_supply); i++) {
			struct regulator_consumer_supply *reg =
						 &palmas_ldousb_supply[i];
			if ((strncmp(reg->supply, "vddio_hv", 8) == 0)
			   && (strncmp(reg->dev_name, "tegradc.1", 9) == 0)) {
				reg->dev_name = "tegradc.0";
				break;
			}
		}
		for (i = 0; i < ARRAY_SIZE(fixed_reg_en_vdd_hdmi_5v0_supply);
				 i++) {
			struct regulator_consumer_supply *reg =
					 &fixed_reg_en_vdd_hdmi_5v0_supply[i];
			if ((strncmp(reg->supply, "vdd_hdmi_5v0", 12) == 0)
			   && (strncmp(reg->dev_name, "tegradc.1", 9) == 0)) {
				reg->dev_name = "tegradc.0";
				break;
			}
