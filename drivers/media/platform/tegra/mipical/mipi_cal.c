			    ...
			    struct tegra_mipi_bias *bias)
{
	...
	if (!ret) {
		bias->pad_driv_up_ref = (v & PAD_DRIV_UP_REF) >> PAD_DRIV_UP_REF_SHIFT;
		bias->pad_driv_dn_ref = (v & PAD_DRIV_DN_REF) >> PAD_DRIV_DN_REF_SHIFT;
	}

	ret = of_property_read_u32(np, "bias-pad-cfg2", &v);
	if (!ret) {
		bias->pad_vclamp_level = (v & PAD_VCLAMP_LEVEL) >> PAD_VCLAMP_LEVEL_SHIFT;
		bias->pad_vauxp_level = (v & PAD_VAUXP_LEVEL) >> PAD_VAUXP_LEVEL_SHIFT;
	}
}
			   ...
			   struct tegra_mipi_prod_dsi *dsi)
{
	...
	if (!ret) {
		dsi->clk_overide_x = (v & CLKOVERIDEDSIA) >> CLKOVERIDEDSIA_SHIFT;
