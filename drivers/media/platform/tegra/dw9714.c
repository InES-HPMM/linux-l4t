...
#define dw9714_ID			0x04
#define dw9714_FOCAL_LENGTH_FLOAT	(4.57f)
#define dw9714_FNUMBER_FLOAT		(2.8f)
#define dw9714_FOCAL_LENGTH		(0x40923D71) /* 4.570f */
#define dw9714_FNUMBER			(0x40333333) /* 2.8f */
#define dw9714_SLEW_RATE			1
#define dw9714_ACTUATOR_RANGE		1023
#define dw9714_SETTLETIME		30
#define dw9714_FOCUS_MACRO		620
#define dw9714_FOCUS_INFINITY		70
#define dw9714_POS_LOW_DEFAULT		16
#define dw9714_POS_HIGH_DEFAULT		1023
#define dw9714_POS_CLAMP		0x03ff
...
dw9714_set_position_fail:
	dev_err(&info->i2c_client->dev,
			"[CAM] DW9714: %s: set position failed\n", __func__);
	return err;
}
...
static int dw9714_power_get(struct dw9714_info *info)
{
	...
	dw9714_regulator_get(info, &pw->vdd, "vdd");
...
static int dw9714_detect(struct dw9714_info *info)
{
	u16 val = 0;
	int err;
...
static void dw9714_get_focuser_capabilities(struct dw9714_info *info)
{
	...
	info->nv_config.pos_actual_low = dw9714_POS_LOW_DEFAULT;
	info->nv_config.pos_actual_high = dw9714_POS_HIGH_DEFAULT;
...
static int dw9714_param_rd(struct dw9714_info *info, unsigned long arg)
{
	...
	case NVC_PARAM_FOCAL_LEN:
		info->nvc.focal_length = dw9714_FOCAL_LENGTH;
		...
		const struct i2c_device_id *id)
{
	...
	if (info->pdata->dev_name != 0)
		...
		strncpy(info->devname, "dw9714", sizeof(info->devname) - 1);
...
{
	struct dw9714_info *k_info = s->private;
...
set_attr:
	...
	case 'p':
		pr_info("new pos = %d\n", val);
		err = dw9714_position_wr(k_info, val);
		if (err)
