...
#if CONFIG_T210_FPGA
	[AR0261_MODE_1952X1096] = mode_1952x1096,
#endif
};
				 ...
				 int num_override_regs)
{
	...
	for (next = table; next->addr != AR0261_TABLE_END; next++) {
		...
		err = vi_i2c_write_direct_test(next->addr, val);
		if (err) {
			pr_err("%s:AR0261_write_table:%d", __func__, err);
			return err;
		}
	}
	return 0;
...
ar0261_set_mode(struct ar0261_info *info, struct ar0261_mode *mode)
{
	...
	if ((mode->xres == 1920) && (mode->yres == 1080)) {
		...
		sensor_mode = AR0261_MODE_1952X1096;
	} else if (mode->xres == 1920 && mode->yres == 35) {
		sensor_mode = AR0261_MODE_1952X1096;
#endif
	} else {
