...
static int ov13860_power_on(struct camera_common_data *s_data)
{
	...
	/* 5ms + 8192 xclk cycles as per the latest datasheet*/
...
static int ov13860_set_group_hold(struct ov13860 *priv)
{
	int err;
...
static int ov13860_otp_setup(struct ov13860 *priv)
{
	...
	otp_buf = devm_kzalloc(&priv->i2c_client->dev,
			   OV13860_OTP_SIZE, GFP_KERNEL);
	if (!otp_buf) {
		dev_err(&priv->i2c_client->dev, "Failed to allocate otp buff\n");
		return -ENOMEM;
	}
...
static int ov13860_fuse_id_setup(struct ov13860 *priv)
{
	...
	/* Here we read sensor ID from OTP. Actually, it can also
...
static int ov13860_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_GROUP_HOLD:
		if (switch_ctrl_qmenu[ctrl->val] == SWITCH_ON) {
			...
			err = ov13860_set_group_hold(priv);
		}
		break;
	case V4L2_CID_EEPROM_DATA:
		if (!ctrl->string[0])
...
static int ov13860_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct v4l2_subdev_internal_ops ov13860_subdev_internal_ops = {
	.open = ov13860_open,
};

static const struct media_entity_operations ov13860_media_ops = {
			...
			const struct i2c_device_id *id)
{
	...
	sprintf(debugfs_name, "ov13860_%c", common_data->csi_port + 'a');
	dev_dbg(&client->dev, "%s: name %s\n", __func__, debugfs_name);
	camera_common_create_debugfs(common_data, debugfs_name);

	v4l2_i2c_subdev_init(priv->subdev, client, &ov13860_subdev_ops);
	...
	priv->subdev->internal_ops = &ov13860_subdev_internal_ops;
	...
	priv->subdev->entity.ops = &ov13860_media_ops;
	...
	dev_dbg(&client->dev, "Detected OV13860 sensor\n");
...
MODULE_DESCRIPTION("I2C driver for Omnivison OV13860");
