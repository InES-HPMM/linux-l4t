...
static void emc_mr4_poll(unsigned long nothing)
{
	...
	if (!test_mode)
		dram_temp = (u32)tegra_emc_get_dram_temperature();
	else
...
static int tegra_dram_temp_start(void)
{
	...
	err = mod_timer(&emc_timer_mr4,
			jiffies + msecs_to_jiffies(timer_period_mr4));
	if (err)
		return err;

	atomic_set(&mr4_do_poll, 1);
	return mod_timer(&emc_timer_mr4,
			 jiffies + msecs_to_jiffies(timer_period_mr4));
}
...
int tegra_emc_timers_init(struct dentry *parent)
{
	...
	if (tegra_emc_get_dram_type() == DRAM_TYPE_LPDDR4) {
		pr_info("tegra: Starting EMC periodic training timer\n");
		setup_timer(&emc_timer_training, emc_train, 0);
		mod_timer(&emc_timer_training, timer_period_training);
	}
#endif
	setup_timer(&emc_timer_mr4, emc_mr4_poll, 0);
