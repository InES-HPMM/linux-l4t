...
struct mc_client mc_clients[] = {
	...
	dummy_client,
	dummy_client,
	client("nvdec", "csr_nvdecsrd"),
	client("nvdec", "csw_nvdecswr"),
	client("ape", "csr_aper"),
	client("ape", "csw_apew"),
	dummy_client,
	dummy_client,
	client("nvjpg", "csr_nvjpgsrd"),
	client("nvjpg", "csw_nvjpgswr"),
	client("se", "csr_sesrd"),
	client("se", "csw_seswr"),
	client("axiap", "csr_axiapr"),
	client("axiap", "csw_axiapw"),
	client("etr", "csr_etrr"),
	client("etr", "csw_etrw"),
	client("tsecb", "csr_tsecsrdb"),
	client("tsecb", "csw_tsecswrb"),
	client("gpu", "csr_gpusrd2"),
	client("gpu", "csw_gpuswr2"),
};
int mc_client_last = ARRAY_SIZE(mc_clients) - 1;
/*** Done. ***/

static void mcerr_t21x_info_update(struct mc_client *c, u32 stat)
{
	...
	if (stat & MC_INT_DECERR_MTS)
		c->intr_counts[6]++;
	if (stat & MC_INT_DECERR_GENERALIZED_CARVEOUT)
		c->intr_counts[7]++;
...
#define fmt_hdr \
	"%-18s %-18s %-9s %-9s %-9s %-10s %-10s %-10s %-10s %-10s %-9s\n"
#define fmt_cli \
	"%-18s %-18s %-9u %-9u %-9u %-10u %-10u %-10u %-10u %-10u %-9u\n"
static int mcerr_t21x_debugfs_show(struct seq_file *s, void *v)
{
	int i, j;
	int do_print;
...
{
	spec->mcerr_info_update = mcerr_t21x_info_update;
	spec->mcerr_debugfs_show = mcerr_t21x_debugfs_show;
