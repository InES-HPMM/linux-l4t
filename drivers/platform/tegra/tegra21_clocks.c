...
static int tegra21_pllcx_clk_enable(struct clk *c)
{
	u32 val;
	pr_debug("%s on clock %s\n", __func__, c->name);
	...
	pllcx_strobe(c);
	tegra21_pll_clk_wait_for_lock(c, c->reg + PLL_BASE,
			PLL_BASE_LOCK | PLLCX_BASE_PHASE_LOCK);
	return 0;
}

static void tegra21_pllcx_clk_disable(struct clk *c)
{
	u32 val;
	pr_debug("%s on clock %s\n", __func__, c->name);
...
static struct clk_ops tegra_pllcx_ops = {
	...
	.enable			= tegra21_pllcx_clk_enable,
	.disable		= tegra21_pllcx_clk_disable,
	.set_rate		= tegra21_pllcx_clk_set_rate,
};
...
static void tegra21_pllxc_clk_init(struct clk *c)
{
	unsigned long input_rate = clk_get_rate(c->parent);
	u32 m, p, val;
...
static int tegra21_pllxc_clk_enable(struct clk *c)
{
	u32 val;
	pr_debug("%s on clock %s\n", __func__, c->name);
...
static void tegra21_pllxc_clk_disable(struct clk *c)
{
	u32 val;
	pr_debug("%s on clock %s\n", __func__, c->name);
...
#define PLLXC_DYN_RAMP(pll_misc, reg)					\
	do {								\
		...
		clk_writel(misc, (reg));				\
		tegra21_pll_clk_wait_for_lock(c, (reg),			\
...
static int tegra21_pllxc_clk_set_rate(struct clk *c, unsigned long rate)
{
	u32 val, pdiv;
	unsigned long input_rate;
	struct clk_pll_freq_table cfg, old_cfg;
...
#ifdef CONFIG_PM_SLEEP
static void tegra21_pllxc_clk_resume_enable(struct clk *c)
{
	unsigned long rate = clk_get_rate_all_locked(c->parent);
	enum clk_state state = c->state;
	...
	rate = clk_get_rate_all_locked(c) + 1;
	tegra21_pllxc_clk_set_rate(c, rate);
	tegra21_pllxc_clk_enable(c);
	c->state = state;
}
#endif

static struct clk_ops tegra_pllxc_ops = {
	.init			= tegra21_pllxc_clk_init,
	.enable			= tegra21_pllxc_clk_enable,
	.disable		= tegra21_pllxc_clk_disable,
	.set_rate		= tegra21_pllxc_clk_set_rate,
};
...
static void tegra21_pllre_clk_init(struct clk *c)
{
	unsigned long input_rate = clk_get_rate(c->parent);
	u32 m, val;
...
static int tegra21_pllre_clk_enable(struct clk *c)
{
	u32 val;
	pr_debug("%s on clock %s\n", __func__, c->name);
	...
	tegra21_pll_clk_wait_for_lock(c, c->reg + PLL_MISC(c), PLLRE_MISC_LOCK);
	return 0;
}

static void tegra21_pllre_clk_disable(struct clk *c)
{
	u32 val;
	pr_debug("%s on clock %s\n", __func__, c->name);
...
static int tegra21_pllre_clk_set_rate(struct clk *c, unsigned long rate)
{
	u32 val, old_base;
	unsigned long input_rate;
	struct clk_pll_freq_table cfg;
...
static struct clk_ops tegra_pllre_ops = {
	.init			= tegra21_pllre_clk_init,
	.enable			= tegra21_pllre_clk_enable,
	.disable		= tegra21_pllre_clk_disable,
	.set_rate		= tegra21_pllre_clk_set_rate,
};

static void tegra21_pllre_out_clk_init(struct clk *c)
{
	u32 p, val;
...
static int tegra21_pllre_out_clk_enable(struct clk *c)
{
	return 0;
}

static void tegra21_pllre_out_clk_disable(struct clk *c)
{
}

static int tegra21_pllre_out_clk_set_rate(struct clk *c, unsigned long rate)
{
	u32 val, p, pdiv;
...
static struct clk_ops tegra_pllre_out_ops = {
	.init			= tegra21_pllre_out_clk_init,
	.enable			= tegra21_pllre_out_clk_enable,
	.disable		= tegra21_pllre_out_clk_disable,
	.set_rate		= tegra21_pllre_out_clk_set_rate,
};

#ifdef CONFIG_PM_SLEEP
/* Resume both pllre_vco and pllre_out */
static void tegra21_pllre_clk_resume_enable(struct clk *c)
{
	u32 pdiv;
	u32 val = clk_readl(c->reg + PLL_BASE);
	...
	tegra21_pllre_clk_set_rate(c->parent, rate);
	...
	tegra21_pllre_clk_enable(c->parent);
...
static struct clk_mux_sel mux_pllc2_c_c3_pllp_plla[] = {
...
static struct clk_mux_sel mux_pllc_pllp_plla_pllc2_c3_clkm[] = {
	{ .input = &tegra_pll_c, .value = 1},
	{ .input = &tegra_pll_p, .value = 2},
	{ .input = &tegra_pll_a_out0, .value = 3},
...
struct clk tegra_list_clks[] = {
	...
	D_AUDIO_CLK("dam0",	"tegra30-dam.0",	NULL,	108,	0x3d8,	153600000,  mux_d_audio_clk,	MUX | DIV_U71 | PERIPH_ON_APB),
	D_AUDIO_CLK("dam1",	"tegra30-dam.1",	NULL,	109,	0x3dc,	153600000,  mux_d_audio_clk,	MUX | DIV_U71 | PERIPH_ON_APB),
	D_AUDIO_CLK("dam2",	"tegra30-dam.2",	NULL,	110,	0x3e0,	153600000,  mux_d_audio_clk,	MUX | DIV_U71 | PERIPH_ON_APB),
	PERIPH_CLK("adx",	"adx",			NULL,   154,	0x638,	153600000, mux_plla_pllc_pllp_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	PERIPH_CLK("adx1",	"adx1",			NULL,   180,	0x670,	153600000, mux_plla_pllc_pllp_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	PERIPH_CLK("amx",	"amx",			NULL,   153,	0x63c,	153600000, mux_plla_pllc_pllp_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	PERIPH_CLK("amx1",	"amx1",			NULL,   185,	0x674,	153600000, mux_plla_pllc_pllp_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	...
	PERIPH_CLK("sata_oob",	"tegra_sata_oob",	NULL,	123,	0x420,	216000000, mux_pllp_pllc_clkm,	MUX | DIV_U71),
	PERIPH_CLK("sata",	"tegra_sata",		NULL,	124,	0x424,	216000000, mux_pllp_pllc_clkm,	MUX | DIV_U71),
	PERIPH_CLK("sata_cold",	"tegra_sata_cold",	NULL,	129,	0,	48000000,  mux_clk_m,			0),
	PERIPH_CLK("vfir",	"vfir",			NULL,	7,	0x168,	408000000,  mux_pllp_pllc_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	PERIPH_CLK("sdmmc1",	"sdhci-tegra.0",	NULL,	14,	0x150,	208000000, mux_pllp_pllc_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	...
	PERIPH_CLK("vde",	"vde",			NULL,	61,	0x1c8,	600000000, mux_pllp_pllc2_c_c3_clkm,	MUX | MUX8 | DIV_U71 | DIV_U71_INT),
	...
	PERIPH_CLK("mipi",	"mipi",			NULL,	50,	0x174,	408000000,  mux_pllp_pllc_clkm,	MUX | DIV_U71 | PERIPH_ON_APB),
	...
	PERIPH_CLK("vic03",	"vic03",		NULL,	178,	0x678,	500000000, mux_pllc_pllp_plla_pllc2_c3_clkm,	MUX | MUX8 | DIV_U71),
#endif
	...
	PERIPH_CLK("hdmi",	"hdmi",			NULL,	51,	0x18c,	594000000, mux_pllp_plld_plla_pllc_plld2_clkm,	MUX | MUX8 | DIV_U71),
	PERIPH_CLK("disp1",	"tegradc.0",		NULL,	27,	0x138,	600000000, mux_pllp_plld_plla_pllc_plld2_clkm,	MUX | MUX8),
	PERIPH_CLK("disp2",	"tegradc.1",		NULL,	26,	0x13c,	600000000, mux_pllp_plld_plla_pllc_plld2_clkm,	MUX | MUX8),
	PERIPH_CLK_EX("sor0",	"sor0",			NULL,	182,	0x414,	408000000, mux_pllp_plld_plla_pllc_plld2_clkm,	MUX | MUX8 | DIV_U71, &tegra_sor_clk_ops),
	...
	PERIPH_CLK("hdmi_audio", "hdmi_audio",		NULL, 176,	0x668,	408000000,  mux_pllp_pllc_clkm,		MUX | MUX8 | DIV_U71 | PERIPH_NO_RESET),
	...
	PERIPH_CLK("se",	"se",			NULL,	127,	0x42c,	600000000, mux_pllp_pllc2_c_c3_clkm,	MUX | MUX8 | DIV_U71 | DIV_U71_INT | PERIPH_ON_APB),
	...
	PERIPH_CLK("soc_therm",	"soc_therm",		NULL,   78,	0x644,	408000000, mux_pllc_pllp_plla_v2,	MUX | MUX8 | DIV_U71 | PERIPH_ON_APB),
...
static void tegra21_clk_resume(void)
{
	...
	tegra21_pllss_clk_resume_enable(&tegra_pll_d2);
	tegra21_pllss_clk_resume_enable(&tegra_pll_dp);
	tegra21_pllcx_clk_resume_enable(&tegra_pll_c2);
	tegra21_pllcx_clk_resume_enable(&tegra_pll_c3);
	tegra21_pllxc_clk_resume_enable(&tegra_pll_c);
	tegra21_pllxc_clk_resume_enable(&tegra_pll_x);
	...
	p = &tegra_pll_c2;
	if (p->state == OFF)
		tegra21_pllcx_clk_disable(p);
	p = &tegra_pll_c3;
	if (p->state == OFF)
		tegra21_pllcx_clk_disable(p);
	p = &tegra_pll_c;
	if (p->state == OFF)
		tegra21_pllxc_clk_disable(p);
	p = &tegra_pll_x;
	if (p->state == OFF)
		tegra21_pllxc_clk_disable(p);
	p = &tegra_pll_re_vco;
	...
	tegra21_pll_clk_init(&tegra_pll_u); /* Re-init utmi parameters */
