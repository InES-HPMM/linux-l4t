...
static void pll_do_enable(struct clk *c)
{
	u32 val = clk_readl(c->reg + PLL_BASE);
	val &= ~PLL_BASE_BYPASS;
	val |= PLL_BASE_ENABLE;
...
static void pll_do_disable(struct clk *c)
{
	u32 val = clk_readl(c->reg + PLL_BASE);
	val &= ~(PLL_BASE_BYPASS | PLL_BASE_ENABLE);
	pll_writel_delay(val, c->reg + PLL_BASE);
...
static int tegra21_pllcx_clk_enable(struct clk *c)
{
	...
	/* out-of-iddq -> out-of-reset -> enable */
	pll_do_iddq(c, c->u.pll.misc1, PLLCX_MISC1_IDDQ, false);

	val = clk_readl(c->reg + c->u.pll.misc0);
	val &= ~PLLCX_MISC0_RESET;
	pll_writel_delay(val, c->reg + c->u.pll.misc0);

	pll_do_enable(c);
	tegra21_pll_clk_wait_for_lock(c, c->reg + PLL_BASE, PLLCX_BASE_LOCK);
	return 0;
}
...
static void tegra21_pllcx_clk_disable(struct clk *c)
{
	...
	pll_do_iddq(c, c->u.pll.misc1, PLLCX_MISC1_IDDQ, true);
}
...
struct clk tegra_list_clks[] = {
	...
	PERIPH_CLK("spare1",	"spare1",		NULL,	192,	0,	26000000,  mux_clk_m,			0),
	PERIPH_CLK("axiap",	"axiap",		NULL,	196,	0,	26000000,  mux_clk_m,			0),
	PERIPH_CLK("sor_safe",	"sor_safe",		NULL,	222,	0,	600000000,  mux_clk_m,			PERIPH_NO_RESET),
	PERIPH_CLK("iqc1",	"iqc1",			NULL,	221,	0,	36864000,  mux_clk_m,			PERIPH_NO_RESET | PERIPH_ON_APB),
	PERIPH_CLK("iqc2",	"iqc2",			NULL,	220,	0,	36864000,  mux_clk_m,			PERIPH_NO_RESET | PERIPH_ON_APB),
	PERIPH_CLK("kfuse",	"kfuse-tegra",		NULL,	40,	0,	26000000,  mux_clk_m,			PERIPH_ON_APB),
	PERIPH_CLK("fuse",	"fuse-tegra",		"fuse",	39,	0,	26000000,  mux_clk_m,			PERIPH_ON_APB),
	PERIPH_CLK("fuse_burn",	"fuse-tegra",		"fuse_burn",	39,	0,	26000000,  mux_clk_m,		PERIPH_ON_APB),
	PERIPH_CLK("apbif",	"tegra210-admaif",	NULL, 107,	0,	26000000,  mux_clk_m,			PERIPH_ON_APB),
...
static void tegra21_clk_resume(void)
{
	...
	tegra21_pllcx_clk_resume_enable(&tegra_pll_c);
	tegra21_pllcx_clk_resume_enable(&tegra_pll_a1);
