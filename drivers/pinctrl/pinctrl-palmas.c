...
PULL_UP_DN(GPADC_START, PU_PD_OD, PALMAS_PU_PD_INPUT_CTRL1, 0x30, 0x0,
		0x20, 0x10);
...
PIN_INFO(GPIO9,		GPIO,		&pu_pd_GPIO9_info,	NULL);
PIN_INFO(GPIO10,	GPIO,		&pu_pd_GPIO10_info,
		&od_GPIO10_info);
PIN_INFO(GPIO11,	GPIO,		&pu_pd_GPIO11_info,	NULL);
PIN_INFO(GPIO12,	GPIO,		NULL,			NULL);
PIN_INFO(GPIO13,	GPIO,		&pu_pd_GPIO13_info,
		&od_GPIO13_info);
PIN_INFO(GPIO14,	GPIO,		&pu_pd_GPIO14_info,	NULL);
PIN_INFO(GPIO15,	GPIO,		NULL,			NULL);
PIN_INFO(ID,		ID,		&pu_pd_ID_info,		NULL);
...
static struct palmas_pinctrl_info palmas_pinctrl_info[PALMAS_PIN_NAME_MAX] = {
	PALMAS_PIN(GPIO0, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD1, 0x04, 0x2,
		&pin_GPIO0_info, &pin_ID_info, NULL, NULL),
	PALMAS_PIN(GPIO1, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD1, 0x18, 0x3,
		&pin_GPIO1_info, &pin_VBUS_DET_info, &pin_LED1_info,
		&pin_PWM1_info),
	PALMAS_PIN(GPIO2, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD1, 0x60, 0x5,
		&pin_GPIO2_info, &pin_REGEN_info, &pin_LED2_info,
		&pin_PWM2_info),
	PALMAS_PIN(GPIO3, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD1, 0x80, 0x7,
		&pin_GPIO3_info, &pin_CHRG_DET_info, NULL, NULL),
	PALMAS_PIN(GPIO4, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD2, 0x01, 0x0,
		&pin_GPIO4_info, &pin_SYSEN1_info, NULL, NULL),
	PALMAS_PIN(GPIO5, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD2, 0x06, 0x1,
		&pin_GPIO5_info, &pin_CLK32KGAUDIO_info, &pin_USB_PSEL_info,
		NULL),
	PALMAS_PIN(GPIO6, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD2, 0x8, 3,
		 &pin_GPIO6_info, &pin_SYSEN2_info, NULL, NULL),
	PALMAS_PIN(GPIO7, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD2, 0x30, 4,
		&pin_GPIO7_info, &pin_MSECURE_info, &pin_PWRHOLD_info, NULL),
	PALMAS_PIN(GPIO8, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD4, 0x01, 0,
		&pin_GPIO8_info, &pin_SIM1RSTI_info, NULL, NULL),
	PALMAS_PIN(GPIO9, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD4, 0x02, 1,
		&pin_GPIO9_info, &pin_LOW_VBAT_info, NULL, NULL),
	PALMAS_PIN(GPIO10, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD4, 0x04, 2,
		&pin_GPIO10_info, &pin_WIRELESS_CHRG1_info, NULL, NULL),
	PALMAS_PIN(GPIO11, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD4, 0x08, 3,
		&pin_GPIO11_info, &pin_RCM_info, NULL, NULL),
	PALMAS_PIN(GPIO12, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD4, 0x10, 4,
		&pin_GPIO12_info, &pin_SIM2RSTO_info, NULL, NULL),
	PALMAS_PIN(GPIO13, NONE, 0, 0, 0,
		&pin_GPIO13_info, NULL, NULL, NULL),
	PALMAS_PIN(GPIO14, NONE, 0, 0, 0,
		&pin_GPIO14_info, NULL, NULL, NULL),
	PALMAS_PIN(GPIO15, PU_PD_OD, PALMAS_PRIMARY_SECONDARY_PAD4, 0x80, 7,
...
static int __devinit palmas_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "SECONDARY_PAD4 read failed %d\n", ret);
		return ret;
	}
